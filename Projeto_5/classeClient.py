...
class Client:
    ...
    def head(self, messageType, idSensor, idServer, nTotalPack, nCurrentPack, handshakeOrData, reSend, lastSucessPack, CRC):
        h0 = messageType
        h1 = idSensor
        h2 = idServer
        h3 = nTotalPack
    ...
    def clientLog(self,action,type,size,sentPckg,totalPckg, CRC):
        char = " / "
        if type == 3:
            log = str(self.getDataClient())+ char + action + char + "3" + char + str(size) + char + str(sentPckg) + char + str(totalPckg)  + char + CRC.decode('ascii')
        else:
            log = str(self.getDataClient())+ char + action + char + str(type) + char + str(size) 

        return log
    
    def datagramaHandshake(self):
        Head = self.head(self.messageType1, self.idSensor, self.idServer, self.byteVazio, self.byteVazio, self.archiveId, self.byteVazio, self.byteVazio, self.crcVazio)
    ...
    def sendPackages(self, packageList):
        ...
        while n < (len(packageList)):
            ...
            if self.forcarErroNbytes:
                ...
                crc = binascii.crc_hqx(payload_eop[0:len(packageList[n])], 0).to_bytes(2,'big')
            ...
            self.ClientLog.append(self.clientLog("envio", int.from_bytes(self.messageType3, byteorder="big"), len(pacote), int.from_bytes(pacote[4:5], byteorder="big"), int.from_bytes(pacote[3:4], byteorder="big"), binascii.hexlify(crc)))
            ...
            if n != (len(packageList)-1):
                ...
                if cronometroTimer2 >=20:
                    timeout = self.head(self.messageType5, self.idSensor, self.idServer, self.byteVazio, self.byteVazio, self.byteVazio, self.byteVazio, self.byteVazio, self.crcVazio)+ self.EOP
